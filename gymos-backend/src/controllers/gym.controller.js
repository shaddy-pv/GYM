...
const updatePointsConfig = async (req, res, next) => {
  try {
    ...
    return successResponse(res, 'Points configuration updated', gym);
  } catch (error) {
    next(error);
  }
};
...
const getGymDashboard = async (req, res, next) => {
  try {
    ...
    const [
      ...
      Payment.aggregate([
        { $match: { gym: gymId, status: 'paid', createdAt: { $gte: monthStart, $lte: monthEnd } } },
    ...
    const monthlyRevenue = await Promise.all(
      last6Months.map(async ({ month, year, start, end }) => {
        ...
        return { month, year, revenue: result[0]?.total || 0 };
      }),
    );
